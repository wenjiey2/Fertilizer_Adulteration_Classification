...
class LinearCentroidsInitialisation(AbstractCentroidsInitialisation):
  ...
  def get_cluster_centroids(self):
    ...
    negative_clusters = int(tf.round(tf.abs(weight_min) / (tf.abs(weight_min) + weight_max) * self.number_of_clusters))
    negative_cluster_centroids = tf.linspace(weight_min, 0.0, negative_clusters)
    negative_cluster_centroids.numpy()
    negative_cluster_centroids = np.delete(negative_cluster_centroids, -1)
    positive_cluster_centroids = tf.linspace(0.0, weight_max, self.number_of_clusters - negative_clusters + 1)
    positive_cluster_centroids.numpy()
    cluster_centroids = np.concatenate((negative_cluster_centroids, positive_cluster_centroids), axis=None)
    cluster_centroids = tf.convert_to_tensor(cluster_centroids)
...
def apply_clustering(layer):
    if isinstance(layer, tf.keras.layers.Conv2D):
        if layer.name == 'conv2d_23':
            return cc.cluster_weights(layer, 256, CentroidInitialization.LINEAR)
        elif layer.output_shape[-2] == 5:
            return cc.cluster_weights(layer, 128, CentroidInitialization.LINEAR)
    elif isinstance(layer, tf.keras.layers.Dense):
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print(cc.cluster_weights(layer, 16, CentroidInitialization.LINEAR))
        return cc.cluster_weights(layer, 16, CentroidInitialization.LINEAR)
    return layer
