...
def apply_quantization_to_custom_layers(layer):
    if isinstance(layer, tf.keras.layers.Conv2D):
        if layer.output_shape[-2] == 20 and layer.output_shape[-1] >= 192:
            ...
            return tfmot.quantization.keras.quantize_annotate_layer(layer, MyPruneQuantizeConfig(16, 16, True))
        ...
        if layer.output_shape[-2] == 40:
            return tfmot.quantization.keras.quantize_annotate_layer(layer, MyPruneQuantizeConfig(8, 10, True))
        elif layer.output_shape[-2] == 20:
            return tfmot.quantization.keras.quantize_annotate_layer(layer, MyPruneQuantizeConfig(6, 8, True))
        elif layer.output_shape[-2] == 10 or layer.output_shape[-2] == 5:
...
def main():
    ...
    for i in range(3):
        ...
        if not os.path.exists("quantized_model.h5"):
            base_model = keras.models.load_model("bl_model.h5",
        ...
        my_callbacks = [
            ...
            pm.MyThresholdCallback(tr_threshold=0.985, val_threshold=0.995),
