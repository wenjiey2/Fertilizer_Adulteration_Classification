...
def cluster_scope():
  ...
  return CustomObjectScope(
      {
          'ClusterWeights': cw.ClusterWeights
      }
  )
...
def cluster_weights(to_cluster,
  ...
  """
  if not cm.CentroidsInitializerFactory.\
  ...
  def _add_clustering_wrapper(layer):
    ...
    if isinstance(layer, cw.ClusterWeights):
      return layer
    if isinstance(layer, InputLayer):
      return layer.__class__.from_config(layer.get_config())

    return cw.ClusterWeights(layer,number_of_clusters, cluster_centroids_init, **kwargs)
...
def strip_clustering(model):
  ...
  def _strip_clustering_wrapper(layer):
    ...
    elif isinstance(layer, cw.ClusterWeights):
