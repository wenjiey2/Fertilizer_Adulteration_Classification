...
def neck_module2(ip, start_step, end_step, prune, decay):
    input = Mish()(ip)
    dws = prune(keras.layers.SeparableConv2D(96, (3, 3), padding='same'), decay(0.4, 0.7, start_step, end_step))(input)
...
def main():
    ...
    for i in range(3):
        ...
        lr_schedule = CustomSchedule(initial_learning_rate=0.001*(0.98**decay_exp)/(i+1),
                                    decay_steps=584,
                                    decay_rate=0.95,
        ...
        if os.path.exists("pruned_model.h5"):
            pruned_model = keras.models.load_model("pruned_model.h5", custom_objects={'Mish': Mish,
